...
const TwistMessageSize = 48

// ErrInvalidMessageSize indicates the message size doesn't match expected format.
var ErrInvalidMessageSize = errors.New("invalid twist message size: expected 48 bytes")
...
// NewTwistMessage creates a new TwistMessage with zero velocities.
...
// Returns a 48-byte slice in little-endian format.
//
// Example:
//
...
// Returns an error if the data size is not exactly 48 bytes.
//
// Example:
//
...
func DecodeTwist(data []byte) (*TwistMessage, error) {
	if len(data) != TwistMessageSize {
		return nil, fmt.Errorf("%w: got %d bytes", ErrInvalidMessageSize, len(data))
	}
...
func (t *TwistMessage) String() string {
	return fmt.Sprintf(
		"Twist{linear: [%.3f, %.3f, %.3f], angular: [%.3f, %.3f, %.3f]}",
